...
%column 18=SOA, column 19=position oddball, column 20 = control encoding
%position, 21 = control 
...
emotional_v1_CRP(:,1:21)=cellfun(@num2str,emo_ctrl(:,1:21),'UniformOutput',false);
...
emotional_v1_CRP(:,1:21)=cellfun(@num2str,emotional_v1_CRP(:,1:21),'UniformOutput',false);

%find trials where odd Remembered 
for j=1:length(emotional_v1_CRP)
    oddnum=emotional_v1_CRP(j,20);
    if any(strcmp(emotional_v1_CRP(j,1:14),oddnum));
        Remembered_emotional_v1_CRP(j,1:21)=emotional_v1_CRP(j,1:21); %create empty rows when the oddball is not Remembered
    end 
    j=j+1;
end 
...
%cd(dir)
%writematrix(E_CRP_all,'control_E_CRP_all.csv');
%writematrix(E_all_odd_forgot_CRP,'control_E_CRP_all_forgot.csv');
%writematrix(E_all_odd_rem_CRP,'control_E_CRP_all_rem.csv');
...
%Delete rows in which the control was not Remembered, emotional
for j=1:length(emotional_v1_CRP)
    oddnum=emotional_v1_CRP(j,20);
    if any(strcmp(emotional_v1_CRP(j,1:14),oddnum));
        Remembered_emotional_v1_CRP(j,1:21)=emotional_v1_CRP(j,1:21); %create empty rows when the control is not Remembered
    end 
    j=j+1;
end 
...
%cd(dir)
%writematrix(lag_crp_subj, 'control_E_lag_crp_subj_before.csv')
%writematrix(lag_crp_after_subj,'control_E_lag_crp_subj_after.csv')
...
dir= '/Users/albaperis/Desktop/Alba/PhDUPM /Von Restroff WP3/Paper_github/Odd_SOA_CRP/Code';
dir_func= '/Users/albaperis/Desktop/Alba/PhDUPM /Von Restroff WP3/September2020/behavioral_toolbox_release1.01'; %Add here the path to the functions from http://memory.psych.upenn.edu/Behavioral_toolbox
cd(dir)
...
perceptual_v1_CRP(:,1:21)=cellfun(@num2str,per_ctrl(:,1:21),'UniformOutput',false);
...
perceptual_v1_CRP(:,1:21)=cellfun(@num2str,perceptual_v1_CRP(:,1:21),'UniformOutput',false);

%find trials where odd Remembered 
for j=1:length(perceptual_v1_CRP)
    oddnum=perceptual_v1_CRP(j,20);
    if any(strcmp(perceptual_v1_CRP(j,1:14),oddnum));
        Remembered_perceptual_v1_CRP(j,1:21)=perceptual_v1_CRP(j,1:21); %create empty rows when the oddball is not Remembered
    end 
    j=j+1;
end 
...
% writematrix(P_CRP_all,'control_P_CRP_all.csv');
% writematrix(P_all_odd_forgot_CRP,'control_P_CRP_all_forgot.csv');
% writematrix(P_all_odd_rem_CRP,'control_P_CRP_all_rem.csv');
...
% cd(dir)
% writematrix(lag_crp_subj, 'control_P_lag_crp_subj_before.csv')
% writematrix(lag_crp_after_subj,'control_P_lag_crp_subj_after.csv')
...
%writetable(collapsed, 'control_collapsed_CRP_omitnan.csv');
...
%cd '/Raw_Results';
%writetable (CRP_all_R, 'control_CRP_all_R.csv');
%writetable(rvf_CRP_R, 'control_rvf_CRP_R.csv');
%writetable(ToFrom_CRP_R, 'control_ToFrom_CRP_R.csv');

%% For R to compare to v. from transitions from controls vs. oddballs 
clearvars; close all; clc 
cd '/Raw_Results'

control = readtable('control_collapsed_CRP_omitnan.csv'); 
oddball = readtable('collapsed_CRP_omitnan.csv'); 

control(:,6) = repmat({'control'}, 560,1); 
oddball(:,6) = repmat({'oddball'}, 560,1); 

dat = [control; oddball]; 
dat.Properties.VariableNames{'Var6'} = 'item_type'; 

%writetable(dat, 'CRP_avgd_oddballvcontrol.csv'); 